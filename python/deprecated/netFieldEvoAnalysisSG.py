...
class Data:
	...
	def __init__(self, path, savePath=None, nRead=None):
		...
		self.rIn = 1.0
		self.rOut = 10.0
		self.riBuffer1 = self.getrindex(self.rIn)
		self.riBuffer2 = self.getrindex(self.rOut)
		self.Omega = sgrid[2]
		...
		rootGM = 2.0*3.14159
		self.data.append((np.power(self.data[1],-1))*self.data[15]*(3.14159*np.square(self.r))/(np.power(rootGM,2))); self.header.append(r"$Q^{-1}$")
		...
		self.mtot = np.sum(self.data[21]*2.0*3.14159*self.r*self.dr, axis=1)
		self.Bztot = np.sum(self.data[12]*2.0*3.14159*self.r*self.dr, axis=1)
		self.Bztot2 = np.sum(self.data[12][...,self.riBuffer1:self.riBuffer2]*2.0*3.14159*self.r[self.riBuffer1:self.riBuffer2]*self.dr[self.riBuffer1:self.riBuffer2], axis=1)
	...
	def getLinSpacedTimeList(self, nProfiles):
		...
		tArray = np.zeros(nProfiles+1)
		for n in range(nProfiles+1):
			tArray[n] = dt * n 
		return tArray
	...
	def profile(self, col, time, vLineCoords=[1.0, 100.0], hLineCoords=[], logOption=0, save=None, savePath=None, legendLabel=None, ymin=None, ymax=None, color=None):
		...
		if logOption==0 and save is not None:
			plt.semilogx(rPlot, plotData, color=color);	plt.ylabel(self.header[col]);
		if logOption==1 and save is not None:
			plt.loglog(rPlot, np.absolute(plotData), color=color);	plt.ylabel(self.header[col]);
		if logOption==0 and save is None:
			plt.semilogx(rPlot, plotData, label=legendLabel, color=color);	plt.ylabel(self.header[col]);
		if logOption==1 and save is None:
			plt.loglog(rPlot, np.absolute(plotData), label=legendLabel, color=color);	plt.ylabel(self.header[col]);
	...
	def multiProfile(self, col, nProfiles, spacing="log", vLineCoords1=[], hLineCoords1=[], logOption=0, save=None, savePath=None, ymin=None, ymax=None):
		...
		for n in range(len(tList)-1):
			self.profile(col, tList[n], logOption=logOption, legendLabel="t = " + str(np.round(self.t[self.gettindex(tList[n])],1)), ymin=ymin, ymax=ymax, vLineCoords=vLineCoords1, hLineCoords=hLineCoords1, color=colorList[n])
		n = nProfiles-1
		self.profile(col, tList[n], logOption=logOption, legendLabel="t = " + str(np.round(self.t[self.gettindex(tList[n])],-2)), ymin=ymin, ymax=ymax, vLineCoords=vLineCoords1, hLineCoords=hLineCoords1, color=colorList[n])
		if save=="png":
	...
	def multiStPlot1(self, tFlip, savePath=None):
		...
		aspect1 = 2.16 * (self.tmax / 30.0) * ((ti2-ti1)/ti2)
		...
		axArray[axNum].set_yticks([0,1,2])		
		...
		maxLog = np.log10(1.1*np.amax(np.abs(self.lum[0:ti2]))) 
		minLog = np.log10(0.9*np.amin(np.abs(self.lum[0:ti2]))) 
		...
		plt.savefig(savePath + "MST1.png", bbox_inches='tight', dpi = 600)
	...
	def multiStPlot2(self, tFlip, savePath=None):
		...
		#aspect1 = 1.65 * (30.0 / self.tmax)
		aspect1 = 1.43 * (self.tmax / 30.0) * ((ti2-ti1)/ti2)
		...
		axNum = 5
		title = self.header[9]		
		plotData = self.data[9][ti1:ti2,10]/self.data[9][self.gettindex(10.0),10]
		axArray[axNum].semilogy(self.t[ti1:ti2], plotData)
		axArray[axNum].set_ylabel(title)
		axArray[axNum].set_xlim([self.t[ti1],self.t[ti2-1]])
		axArray[axNum].set_xlabel("Time")
		maxLog = np.log10(1.1*np.amax(np.abs(plotData[0:ti2]))) 
		minLog = np.log10(0.9*np.amin(np.abs(plotData[0:ti2]))) 
		axArray[axNum].set_ylim([np.power(10, minLog), np.power(10, maxLog)])
		tickVals = []
		for i in range(-5, 6):
			if minLog < i < maxLog:
