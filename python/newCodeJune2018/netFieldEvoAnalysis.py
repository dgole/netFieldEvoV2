...
def logSpacedTimeList(do, nProfiles):
...
def linSpacedTimeList(do, nProfiles):
	dt = do.tmax/(nProfiles)
	tArray = np.asarray([n*dt for n in range(nProfiles+1)])
	return tArray

def profile(do, col, time, logOption=0, figNum=0, legendLabel=None):
	plt.figure(figNum)
	n        = do.gettindex(time)
	plotData = do.data[col][n,:]
	title    = do.header[col]
	rPlot    = do.r[:]
	if logOption==0:
		plt.semilogx(rPlot, plotData, label=legendLabel);	plt.ylabel(title);
	if logOption==1:
		plt.loglog(rPlot, np.absolute(plotData), label=legendLabel);	plt.ylabel(title);
...
def st(do, col, cmapType='coolwarm', logOption=0, clim1=None, clim2=None, xLabel=1, tStart=None, tEnd=None):
	if tStart is None: 
		plotData = do.data[col];
		tPlotMax = do.tmax
	else:
		tiStart  = do.gettindex(tStart)
		tiEnd    = do.gettindex(tEnd)
		plotData = do.data[col][tiStart:tiEnd]
		tPlotMax = tEnd-tStart	
	title = do.header[col];
#	if logOption==1:
#		plt.imshow(np.transpose(np.fliplr(plotData)), extent=[0,tPlotMax,np.log10(do.rmin),np.log10(do.rmax)], aspect=(80*tPlotMax/(do.rmax-do.rmin)), cmap=plt.get_cmap(cmapType), norm=LogNorm(vmin=clim1, vmax=clim2))
#	if logOption==0:
#		plt.imshow(np.transpose(np.fliplr(plotData)), extent=[0,tPlotMax,np.log10(do.rmin),np.log10(do.rmax)], aspect=(80*tPlotMax/(do.rmax-do.rmin)), cmap=plt.get_cmap(cmapType))
	if logOption==2:
		plotData=plotData/np.amax(np.abs(plotData))
		plt.imshow(np.transpose(np.fliplr(plotData)), extent=[0,tPlotMax,np.log10(do.rmin),np.log10(do.rmax)], aspect=(80*tPlotMax/(do.rmax-do.rmin)), cmap=plt.get_cmap(cmapType), norm=colors.SymLogNorm(linthresh=0.01, linscale=0.5, vmin=-1.0, vmax=1.0))
	plt.yscale('log')
	plt.title(title)
