...
def getPsi0(sg):
	...
	bz0 = np.ones_like(sg.r)*bInitScale/1.e10
	bz0[78:98] = bInitScale
...
def getBz0(sg):
	bz0 = np.ones_like(sg.r)*bInitScale/1000.0
	bz0[78:98] = bInitScale
	return bz0

# alpha calculation 
def getAlpha(sg, s):
	alphaRaw = np.ones_like(sg.r)*alphaMaxAz
	alphaSmooth = alphaRaw
	...
	def __init__(self, sg, s, initialize=0):
		if initialize==1:
			if bInitScale < 0.0: 
				...
				self.alphaSmooth = self.alphaRaw
